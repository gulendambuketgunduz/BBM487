            ...
            else
            {
                ...
                if (user_control == null || user_control.Rows.Count == 0)
                {
                    ...
                    string insertCmd = "INSERT INTO tblUser VALUES('" + tbxName.Text + "','" + tbxSurname.Text + "'," +
                        ...
                        "'" + userName + "','" + userPassword + "','" + userTc + "','" + userEmail + "','" + userCaption + "','" + userAdress + "',0,1,0)";

                    /*At least one record from the transaction needs to be affected-affected row count*/
                    if (DBOperations.insertUpdateDelete(insertCmd) > 0)/*If all the information is entered correctly*/
                    {
